...
outToFile = open('wholehandbook.txt', mode='w')
outToFile.write(''+'\n'+'\n')
outToFile.close()
...
def extractlinks(blspage):
    ...
    sets = soup.find_all('p')
    patFinder2 = re.compile('<.*?>')
    subFoundhtml = patFinder2.sub('', str(sets))


    #file creator
    
    patFinder1 = re.compile('print/.*?.htm')
    findPat1 = re.findall(patFinder1, url)
    patFinder2 = re.compile('print/')
    subFound = patFinder2.sub('', str(findPat1))
    #f = open(str(subFound)+'.txt','w')
    #f.close()
    
    
    outToFile = open(str(subFound)+'.txt', mode='w')
    outToFile.write(str(subFoundhtml)+'\n'+'\n')
    outToFile.close()
    outToFile = open('wholehandbook.txt', mode='a+')
    outToFile.write(str(subFoundhtml)+'\n'+'\n')
    outToFile.close()
